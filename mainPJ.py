...
print("\nModel Comparison")
print("Linear R^2:", lin_model.score(x_test_scaled, y_test))
print("Ridge  R^2:", ridge.score(x_test_scaled, y_test))
print("Lasso  R^2:", lasso.score(x_test_scaled, y_test))
print("Poly(2) R^2:", poly_model.score(x_test_poly, y_test))
print("SVR    R^2:", svr.score(x_test_scaled, y_test))

# Plot comparison
plt.figure(figsize=(10,5))
plt.plot(y_test.index, y_test, label="Actual")
plt.plot(y_test.index, lin_preds, label="Linear")
plt.plot(y_test.index, ridge_preds, label="Ridge")
plt.plot(y_test.index, lasso_preds, label="Lasso")
plt.plot(y_test.index, poly_preds, label="Poly(2)")
plt.plot(y_test.index, svr_preds, label="SVR")
plt.legend()
plt.title("5-Day Forward Return Prediction (Model Comparison)")
